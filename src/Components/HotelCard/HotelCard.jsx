...
const HotelCard = ({room,index}) => {
    ...
    return (
       <Link to={`/rooms/`+room._id} onClick={()=>scrollTo(0,0)} key={room._id} className='relative max-w-70 lg:max-w-60 w-full rounded-xl overflow-hidden bg-white text-gray-500/90 shadow-[0px_4px_4px_rgba(0,0,0,0.5)]'>
      ...
      <p className='px-3 py-1 absolute top-3 left-3 text-xs bg-white text-gray-800 font-medium rounded-full'>Best Seller</p>
       ...
       <div className='p-4 pt-5'>
        ...
        <div className='flex items-center justify-between mt-4'>
            ...
            <button className='px-4 py-2 text-sm font-medium border border-gray-300 rounded hover:bg-gray-50 transition-all curson-pointer'>Book Now</button>
        </div>
       </div>
